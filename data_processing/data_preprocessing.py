...
def prep(dataset_name, target = None, without = [], test_size = 0.2, model_class = "xgboost"):
    if model_class == "xgboost":
        return prep_data(dataset_name, target, without, test_size)
    elif model_class == "rule_ensambe":
        return prep_data_rule_learner_better(dataset_name, target, without, test_size)
    else:
        return ValueError


def prep_data_rule_learner_better(dataset_name, target = None, without = [], test_size = 0.2):
    """
     >>> fn1 = "titanic"
     >>> target = "Survived"
